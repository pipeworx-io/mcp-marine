...
`tools/list` at `https://gateway.pipeworx.io/marine/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/get_wave_forecast \
  -H 'Content-Type: application/json' \
  -d '{"latitude":21.3099,"longitude":-157.8581,"days":5}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/get_wave_forecast`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
